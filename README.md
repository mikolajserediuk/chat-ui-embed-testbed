...
Every option from the integration guide (`width`, `height`, `zIndex`, `allowFullscreen`, `bubbleIconOnly`, offsets, gap) is a form field, editable live before each `init`.
...
1. Set **baseUrl** to your mercury-ui deployment (e.g. `https://localhost:8080` for local dev, or `https://staging.cobundu.com` once the embed path is live there).
2. Paste a valid JWT into the **jwt** field. Never commit a real token.
3. Click **init**.
